...
export default function ProductDetail({ params }: { params: { id: string } }) {
  const product = logoSamples.find((p) => p.id === Number.parseInt(params.id))
  ...
  const handleCopyEmail = () => {
    navigator.clipboard.writeText("contact@example.com") // Default email
    setCopied(true)
  ...
  return (
    <main className="min-h-screen bg-white p-6">
      <div className="max-w-5xl mx-auto">
        {/* Back Button */}
        <Link href="/products" className="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 font-semibold mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Category Badge */}
            <span className="inline-block bg-blue-600 text-white text-xs font-semibold px-3 py-1 rounded-full w-fit">
              {product.title.split(" ")[0]} {/* Temporary category */}
            </span>
          </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="border-l-4 border-blue-600 pl-4">
              <p className="text-xs text-slate-500 font-semibold mb-1">STARTING PRICE</p>
              <p className="text-2xl font-bold text-blue-600">{product.priceRange}</p>
              <p className="text-xs text-slate-500 mt-1">Price varies based on requirements</p>
            </div>

            {/* About section for more detail (Default empty) */}
            <div className="bg-slate-50 rounded p-3 border border-slate-200">
              <p className="text-xs font-semibold text-slate-700 mb-2 uppercase">About This Design</p>
              <p className="text-sm text-slate-600 leading-relaxed">
                {`A unique design tailored for ${product.title.toLowerCase()}. Perfect for various branding needs.`}
              </p>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-1 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold py-2 px-3 rounded transition-colors"
                >
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-1 bg-pink-600 hover:bg-pink-700 text-white text-xs font-semibold py-2 px-3 rounded transition-colors"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-1 bg-slate-900 hover:bg-slate-950 text-white text-xs font-semibold py-2 px-3 rounded transition-colors"
                >
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1 bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold py-2 px-3 rounded transition-colors"
